...
public interface UserRepository extends JpaRepository<User, Long>, JpaSpecificationExecutor<User> {
    ...
    @Query("SELECT u FROM User u LEFT JOIN FETCH u.groups WHERE u.username = :username AND u.deletedAt IS NULL")
    Optional<User> findByUsernameWithGroups(String username);

    @Query("SELECT DISTINCT u FROM User u LEFT JOIN FETCH u.groups WHERE u.id = :id AND u.deletedAt IS NULL")
